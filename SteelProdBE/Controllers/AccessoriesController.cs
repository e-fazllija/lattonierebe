...
namespace SteelProdBE.Controllers
{
    ...
    public class AccessoriesController : ControllerBase
    {
        ...
        [HttpPost]
        [Route(nameof(Create))]
        public async Task<IActionResult> Create(AccessoryCreateModel request)
        {
            try
            {
                AccessorySelectModel Result = await _accessoryServices.Create(request);
                return Ok(Result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [Route(nameof(Update))]
        public async Task<IActionResult> Update(AccessoryUpdateModel request)
        {
            try
            {
                AccessorySelectModel Result = await _accessoryServices.Update(request);
        ...
        public async Task<IActionResult> Get(int currentPage, string? filterRequest)
        {
            try
            {
                ...
                ListViewModel<AccessorySelectModel> res = await _accessoryServices.Get(currentPage, filterRequest, null, null);
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                AccessorySelectModel result = new AccessorySelectModel();
                result = await _accessoryServices.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                Accessory result = await _accessoryServices.Delete(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportExcel(char? fromName, char? toName)
        {
            try
            {
                var result = await _accessoryServices.Get(0, null, fromName, toName);
                DataTable table = Export.ToDataTable<AccessorySelectModel>(result.Data);
